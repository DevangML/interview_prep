...
- **`/upskill`** — aggregate mode: analyses all jobs in `job_hunt/job_search_tracker.csv`, merged with ranked postings (`rank_score >= 45`) from `job_hunt/job_scraper/seen_jobs.json`
...
### Aggregate mode
1. Read `job_hunt/job_search_tracker.csv`. Extract all rows. The columns are:
...
5. Check `job_hunt/upskill/` for the most recent aggregate report file (`report-YYYY-MM-DD.md`) — if one exists, note its date and load it for the diff in Step 8.
...
- **Aggregate:** `job_hunt/upskill/report-YYYY-MM-DD.md`
- **Targeted:** `job_hunt/upskill/report-YYYY-MM-DD-<company-slug>-<role-slug>.md`
  - Slugify: lowercase, spaces → hyphens, strip special characters
  - Example: `job_hunt/upskill/report-2026-04-20-guardsix-senior-ai-engineer.md`
...
3. **Targeted mode ignores both state files.** In targeted mode, analyse only the fetched posting. Do not load or reference `job_hunt/job_search_tracker.csv` or `job_hunt/job_scraper/seen_jobs.json` — both are aggregate-mode-only inputs.
