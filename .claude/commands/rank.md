...
- Nothing → rank all jobs with status `new` in `job_hunt/job_scraper/seen_jobs.json`
...
1. Read `job_hunt/job_scraper/seen_jobs.json`. If the file is missing or has no entries, tell the user to run `/scrape` first and stop.
2. Read `job_hunt/job_search_tracker.csv`. Build the exclusion set: any company+role already in the tracker is out of scope regardless of flags - it has been applied to or consciously tracked.
...
Update `job_hunt/job_scraper/seen_jobs.json` in place - these fields are additive to the scraper's schema:
...
Do not modify `job_hunt/job_search_tracker.csv` - that file records applications, and `/rank` never applies. Re-running `/rank` is idempotent: already-`ranked` jobs are skipped unless `--all` re-scores them.
