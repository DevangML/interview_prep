...
Otherwise, before greeting the user, scan the `job_hunt/documents/` folder. Use Glob with `job_hunt/documents/**/*` and count files per subfolder (`cv/`, `linkedin/`, `diplomas/`, `references/`, `applications/`).

Then welcome the user with a single message that lists three paths. The wording changes based on what was found.

**If `job_hunt/documents/` has files** in one or more subfolders, lead with Path A:
...
>
> I see files in your `job_hunt/documents/` folder: [list per subfolder, e.g. "2 in cv/, 1 in linkedin/, 3 in references/"]. Three ways to start:
>
> **Path A: Read my documents folder** (recommended for what you have) - I'll read everything in `job_hunt/documents/`, cross-reference for consistency, and build your profile from real source materials. Idempotent and safe to re-run as you add more documents.
>
...
**If `job_hunt/documents/` is empty or missing**, surface Path A as a "do this if you have materials" option:
...
> Three ways to start:
>
> **Path A: Documents folder** (best signal if you have several materials) - Drop your CV / LinkedIn export / diplomas / reference letters in the `job_hunt/documents/` folder, then say "go". I'll read everything and build your profile from it. See `job_hunt/documents/README.md` for the folder layout.
>
> **Path B: Single CV import** - Paste or @-mention a single CV/resume here. I'll extract it and ask follow-up questions for what's missing.
>
...
Wait for the user's choice. If they pick A but the folder is still empty, tell them what to add (point at `job_hunt/documents/README.md`) and stop.

---

## Path A: Documents Folder

Reads structured documents in `job_hunt/documents/`, cross-references them for consistency, and merges extracted data into the seven profile skill files. Read-before-write and idempotent: changes already present will not be proposed again.

Follow these steps **exactly in order**.

### Step A1: Inventory

Use Glob with `job_hunt/documents/**/*` to scan the full tree. Print:
...
If every subfolder is empty, stop and tell the user to populate the folder. Point at `job_hunt/documents/README.md` for the layout.
...
**`job_hunt/cv/` documents:** name, contact (email, phone, LinkedIn, GitHub), education (degree, institution, dates, thesis), work experience (title, company, dates, location, bullets), skills, languages (with any stated proficiency), publications, awards, profile/summary.
...
- Languages you work in professionally, with proficiency levels (only if not already extracted from `job_hunt/cv/` or `linkedin/` above) - this feeds the Language Gate in `04-job-evaluation.md`, so ask directly rather than skipping it
...
Then proceed to Step 3 to populate the non-skill files (`CLAUDE.md`, `job_hunt/cv/main_example.tex`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
### 7. Update `job_hunt/cv/main_example.tex`
...
> - `job_hunt/cv/main_example.tex` - Your LaTeX CV template
...
- Three onboarding paths converge on the same skill files. Step 0 picks the right path based on what's in `job_hunt/documents/`. Steps 3 and 4 are shared.
