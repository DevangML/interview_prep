...
- `documents` — deletes user-provided files from the `job_hunt/documents/` folder only
...
>
> - **`documents`** — Deletes all files you've placed in the `job_hunt/documents/` folder (CV PDFs, LinkedIn export, diplomas, references, past applications). The folder structure and `README.md` are preserved.
>
> - **`all`** — Both of the above.
>
...
Use Glob to list all files present in `job_hunt/documents/cv/`, `job_hunt/documents/linkedin/`, `job_hunt/documents/diplomas/`, `job_hunt/documents/references/`, and `job_hunt/documents/applications/`. Present as:

```
## Documents reset will delete:

job_hunt/documents/cv/
  - [filename] or "(empty)"

job_hunt/documents/linkedin/
  - [filename] or "(empty)"

job_hunt/documents/diplomas/
  - [filename] or "(empty)"

job_hunt/documents/references/
  - [filename] or "(empty)"

job_hunt/documents/applications/
  - [subfolder/filename] or "(empty)"

job_hunt/documents/README.md — NOT deleted (instructions file)
```
...
For each non-empty document subfolder, delete all files within it using Bash `rm`. Do not delete the folder itself, and do not delete `job_hunt/documents/README.md`.

```bash
rm -f job_hunt/documents/cv/*
rm -f job_hunt/documents/linkedin/*
rm -f job_hunt/documents/diplomas/*
rm -f job_hunt/documents/references/*
rm -rf job_hunt/documents/applications/*/
```
...
> Your candidate profile is now blank. Run `/setup` to repopulate it. The command auto-detects any files in your `job_hunt/documents/` folder and offers to read from there; otherwise it walks you through a CV import or interactive interview.

**If documents were reset:**
> The `job_hunt/documents/` folder is now empty. Add your career documents and run `/setup` to populate your profile. See `job_hunt/documents/README.md` for instructions on what to put where.

**If both were reset:**
> Both your profile files and documents folder are now empty. Add documents to `job_hunt/documents/` (or skip and use the CV import / interview path), then run `/setup`.
