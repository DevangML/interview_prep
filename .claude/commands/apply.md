...
- Read any existing `job_hunt/cv/main_*<CV_EXT>` file as a structural reference
- Read any existing `job_hunt/cover_letters/cover_*<COVER_EXT>` or `job_hunt/cover_letters/Cover_*<COVER_EXT>` file as a structural reference

*The master candidate profile (`01-candidate-profile.md`), the master CV (`job_hunt/cv/main_example.tex`), and CLAUDE.md's Candidate Profile section are the sole source of truth for facts; existing tailored CVs may be read for structure and phrasing only, never as a source of claims.*
...
### CV (`job_hunt/cv/main_<company>_<role><CV_EXT>`)
...
- Keep to 2 pages
- **Grounding Audit:** Before writing to disk, audit all tailored bullet points against the union of three sources: `.claude/skills/job-application-assistant/01-candidate-profile.md` + the master CV (`job_hunt/cv/main_example.tex`) + `CLAUDE.md`'s Candidate Profile section to verify that all dates, roles, and metrics match exactly (zero profile drift or fabrication).

### Cover Letter (`job_hunt/cover_letters/cover_<company>_<role><COVER_EXT>`)
...
- `.claude/skills/job-application-assistant/03-writing-style.md`
- `.claude/skills/job-application-assistant/04-job-evaluation.md`
- The master CV baseline template (`job_hunt/cv/main_example.tex`)
...
**CV (`job_hunt/cv/main_<company>_<role>.pdf`):**
...
**Cover letter (`job_hunt/cover_letters/cover_<company>_<role>.pdf`):**
- [ ] Exactly 1 page
...
- `job_hunt/cv/main_<company>_<role><CV_EXT>`
- `job_hunt/cover_letters/cover_<company>_<role><COVER_EXT>`
