...
- **With an argument:** match against `job_hunt/job_search_tracker.csv` rows (case-insensitive on company, then role). One match → proceed. Several → list and ask. None → this application isn't tracked; suggest `/outcome <company>` to register it first, or accept the posting and role details directly if the user wants to prep anyway.
...
1. **The archive** (maintained by `/outcome`): `job_hunt/documents/applications/<company>_<role>/`
...
2. **Fallbacks** (the application may predate `/outcome`): posting via WebFetch on the tracker row's `source` URL, or ask the user to paste it; CV via `job_hunt/cv/main_<company>*.tex` and cover letter via `job_hunt/cover_letters/cover_<company>_*.tex`. State plainly which context is missing rather than guessing - and suggest `/outcome <company>` to build the archive for next time.
...
5. **Write only to the application archive** — with one exception. The prep pack lands in `job_hunt/documents/applications/<company>_<role>/`; framework files are not edited, except appending user-approved STAR examples to `07-interview-prep.md` on explicit request.
