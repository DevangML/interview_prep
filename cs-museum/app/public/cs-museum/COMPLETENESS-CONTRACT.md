...
The source research notes contain a separate 17-cluster/70-paradigm narrative. That is an exploratory model, not the release denominator. The canonical release is frozen at 14 clusters and 200 IDs; the three additional narrative clusters are not silently counted as shipped coverage.
...
| Entire bedrock→HCI/UX breadth and depth? | No, current release is a language-concept atlas with selected stack connections | Partial: the layer map is explicit, but depth records, implementation and assessments are open | `SHARED_SPEC.md`, `STACK-COVERAGE.md`, `LEARNING-EVALUATION.md` |
| Every datum diamond-source verified? | No, only cited claims with sources; 5,168 cells are unknown and many records await expert review | Yes for each individual claim only after source-review promotion state is recorded | `EVIDENCE-LEDGER.md`, `SOURCE-QUALITY-AUDIT.md`, `source-manifest.json` |
| Everything downloaded; only React page left? | No | All 104 cited URLs have local hashed snapshots (seven with explicit fallbacks); reader/React implementation, source review and empirical gates remain | `source-manifest.json`, `generated/manifest.json`, `BENCHMARK-PLAN.md` |
