...
## S2 — OFFLINE-FIRST WAREHOUSE SYNC AND CONFLICT CONTROL
...
**Task.** I worked on the offline-first layer of the Next-Gen Field App — the sync and concurrency/conflict-control behaviour that keeps operations correct under low and no network — and I owned documenting the offline design approach for the rest of the team.

**Action.** The core decision was what "offline" is allowed to mean per operation. I treated it as a per-transaction question, not a global mode. Operations that only append — recording a scan, capturing a receipt line — queue locally and reconcile on reconnect. Operations that mutate shared server state — anything where two operators could touch the same bin or the same document — cannot be resolved by last-write-wins, because whoever syncs later silently overwrites real physical work. For those I worked on conflict detection and controlled resolution rather than blind replay, so the conflict surfaces as a decision instead of as corrupted stock. The tradeoff I argued for explicitly: a strict conflict policy means an operator occasionally sees a "this changed underneath you" state, which is friction on the floor. I took that friction, because the alternative failure is invisible and expensive — inventory drift you find at audit, weeks later. I also wrote the design up, because an offline model that only lives in one engineer's head becomes wrong the moment someone else adds a screen.

**Result.** The offline path holds across the full warehouse lifecycle — GRN, putaway, PSR, picking, STN, returns — and the app is live across multiple enterprise SaaS clients. The written design is what new features get checked against, so the model degrades slower than it otherwise would.

**If they probe — "how do you resolve a conflict concretely?"**
> "The general rule I applied: detect on the server using the record's version, not the client's clock — client clocks in a warehouse are unreliable. Then classify. Idempotent and commutative operations replay safely. Non-commutative ones stop and surface to the operator, because a human on the floor can see the physical pallet and the server can't."
