...
**[AD-2] The Concept Atlas is a build-time generated static Directed Acyclic Graph (DAG).**
- **Binds:** The data delivery and fetching mechanisms.
- **Prevents:** Database latency disrupting the 60fps zooming experience.
- **Rule:** The ontology (the ASCII map of CS concepts) is compiled into a strict JSON/FlatBuffer file at build time. It is served entirely via edge CDN. There is no live Graph Database (Neo4j) queried at runtime. Updates to the ontology require a CI/CD rebuild. 
...
- **Rule:** The application must never mount all 10,000 nodes simultaneously. The state manager must track the camera's Z-depth. Only macro-nodes are rendered at high altitudes. Micro-nodes (e.g., ALU logic gates) are physically unmounted from the scene graph until their parent macro-node is focused and the camera crosses the Z-depth threshold.
...
- **Rule:** The LLM does not generate UI components. It generates standard text responses accompanied by strict JSON payloads (e.g., `{"action": "FLY_TO_NODE", "target": "memory_allocation"}`). The React application listens to these events and commands the WebGL camera to animate to the target coordinates. The AI is a passenger that controls the steering wheel, but the engine runs independently.
...
- **Data Schema:** Zod (to validate the massive ontology JSON at build time)
...
The release is structurally buildable at 200 concepts, 14 clusters and 26 language columns. Its evidence ledger records 11 first-class cells, 21 partial cells and 5,168 unknown cells. Unknown is an explicit research backlog. Structural validation and negative contract tests pass; source completeness, expert review, spatial performance and learning efficacy remain open gates.
