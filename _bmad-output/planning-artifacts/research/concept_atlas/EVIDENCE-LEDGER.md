...
The current citation set contains 40 unique URLs. `scripts/download_sources.py` archived all 40 locally with SHA-256 hashes in [source-manifest.json](source-manifest.json). The archive is a reproducible snapshot, not proof that every statement in every source is correct or remains current.
