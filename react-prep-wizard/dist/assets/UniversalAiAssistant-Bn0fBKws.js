...
.metric { font-size: 11px; font-family: monospace; color: #38bdf8; }`}};function ge(o){if(!o.trim())return X;const t=o.toLowerCase();return X.filter(r=>r.title.toLowerCase().includes(t)||r.summary.toLowerCase().includes(t)||r.category.toLowerCase().includes(t)||r.applicableTopics.some(n=>n.toLowerCase().includes(t))||r.keyInvariants.some(n=>n.toLowerCase().includes(t)))}function he(o,t){return{duelTitle:`⚡ Rapid Clash: ${o}`,badge:"Real-Time Dialectic",questions:[{question:`In a high-throughput React system, what is the primary invariant for "${o}"?`,options:["It guarantees O(1) time complexity by caching all intermediate states in localStorage.","It enforces deterministic state transformations while avoiding main-thread Long Animation Frames (LoAF).","It disables the Fiber reconciler tree diffing algorithm completely.","It replaces the browser microtask queue with a native synchronous thread."],correctIndex:1,explanation:`Under the hood, "${o}" aligns with the React 19 concurrent runtime by batching state changes and preserving main-thread responsiveness.`},{question:`When debugging memory leaks related to "${o}", what should you inspect in Chrome DevTools?`,options:["Retainer trees in Heap Snapshots for un-cleaned event listeners and detached DOM nodes.","The CSS style rules panel for unused classes.","The network tab status code for 200 OK headers.","The cookie expiration date timestamps."],correctIndex:0,explanation:"Heap Snapshot retainers reveal root references preventing the V8 Garbage Collector from reclaiming memory."}]}}function fe({contextType:o,roadmapContext:t,projectContext:r,sandboxContext:n,masteryContext:u,chatWithMentor:y,isAiReady:D}){const[I,g]=p.useState([]),[k,v]=p.useState(!1),[b,x]=p.useState(()=>o==="project"?"architect":o==="sandbox"?"copilot":"tutor"),w=p.useRef("");p.useEffect(()=>{const l=`${o}:${(t==null?void 0:t.topicId)||(r==null?void 0:r.projectId)||"main"}`;if(w.current!==l){w.current=l;let i="";if(o==="roadmap"&&(t!=null&&t.topicTitle))i=`👋 **Welcome to the ${t.topicTitle} Socratic Hub!**

I am your **Senior Staff Teaching Architect**. I am grounded in the official specifications and V8/React 19 internals.

*Ask me to break down this mechanism, generate a real-time concept duel, or find authoritative RFC references!*`;else if(o==="project"&&(r!=null&&r.blueprint)){const s=P[r.blueprint.id];i=`🏛️ **Tier-1 Systems Architecture Hub: ${r.blueprint.title}**

I have inside-out mastery of this blueprint:
- **Architecture Pattern**: ${r.blueprint.architecturePattern}
- **Syllabus Coverage**: ${(s==null?void 0:s.syllabusCoveragePercentage)??98}%

*Tell me what you'd like to explore: "Teach me how this works", "Does this cover everything?", "Run a mock defense", or "Suggest extensions"!*`}else o==="sandbox"?i=`🛠️ **Sandbox Compiler & AST Copilot Ready**

I monitor your live JSX/CSS scratchpad and Babel compilation errors in real-time.

*Ask me to diagnose syntax errors, scaffold production-grade components, or optimize React 19 performance!*`:i=`🔮 **Universal AI Problem & Systems Mentor Ready**

How can I accelerate your Staff/Principal interview preparation today?`;g([{id:`welcome-${Date.now()}`,role:"assistant",content:i,timestamp:Date.now(),persona:b}])}},[o,t==null?void 0:t.topicId,r==null?void 0:r.projectId]);const M=p.useCallback(l=>{let i=`[ACTIVE ENVIRONMENT CONTEXT]
Mode: ${o.toUpperCase()} | Persona: ${b.toUpperCase()}
`;if(o==="roadmap"&&t&&(i+=`Track: ${t.trackName||"Core"}
Topic: ${t.topicTitle}
Area: ${t.area}
Summary: ${t.topicSummary||"N/A"}
Key Invariants: ${(t.keyPoints||[]).join("; ")}

`),o==="project"&&(r!=null&&r.blueprint)){const s=r.blueprint,m=P[s.id];i+=`Project ID: ${s.id}
Title: ${s.title}
Analog: ${s.realWorldAnalog}
Architecture: ${s.architecturePattern}
Layers: ${s.layers.map(h=>`${h.layer} (${h.components.join(", ")})`).join(" | ")}
Syllabus Areas: ${m==null?void 0:m.coveredSyllabusAreas.join(", ")}

`}return o==="sandbox"&&n&&(i+=`Active JSX Code:
\`\`\`jsx
${n.jsxCode||"(empty)"}
\`\`\`
Active CSS Code:
...
${n.cssCode||"(empty)"}
\`\`\`
`,n.error&&(i+=`Current Compilation / Runtime Error: ${n.error}

`)),o==="mastery"&&u&&(i+=`Exercise: ${u.unitTitle}
...
Deliver a rigorous, verified, and well-structured response following your system invariants.`},[o,b,t,r,n,u]),C=p.useCallback(l=>{var s,m,h,S;const i=l.toLowerCase().trim();if(o==="project"&&(r!=null&&r.blueprint)){const d=r.blueprint,a=P[d.id];if(i.includes("cover everything")||i.includes("coverage")||i.includes("syllabus")||i.includes("everything"))return`### 🎯 Deep Syllabus Coverage Analysis for **${d.title}**

Yes! **${d.title}** is explicitly architected to achieve **${(a==null?void 0:a.syllabusCoveragePercentage)||100}% syllabus coverage** across core Staff-level frontend and distributed UI concepts:

#### 1. Explicitly Covered Syllabus Core:
${d.explicitTopics.map(c=>`- **${c.topic}** (*${c.subtopic}*): ${c.howCovered}`).join(`
`)}

#### 2. Implicit Foundations Tested (Under the Hood):
${d.implicitFoundations.map(c=>`- **${c.domain} — ${c.title}**: ${c.mechanism} (*Impact: ${c.realWorldImpact}*)`).join(`
`)}
...
1. **${((s=a==null?void 0:a.suggestedExtensions[0])==null?void 0:s.title)||"Multi-device delta streaming"}**
2. **${((m=a==null?void 0:a.suggestedExtensions[1])==null?void 0:m.title)||"Hardware-accelerated compute shaders"}**

*Would you like to drill into any specific layer or step, or run a live mock defense challenge?*`;if(i.includes("teach me")||i.includes("how does it work")||i.includes("explain")||i.includes("start")||i.includes("walkthrough"))return`### 🎓 Deep Architectural Socratic Walkthrough: **${d.title}**
...
${d.stages.map(c=>`**Stage ${c.stageNumber}: ${c.stageName}**
*Focus*: ${c.focus}
*Architectural Lesson*: ${c.architecturalLesson}
${c.codeSnippet?`\`\`\`typescript
${c.codeSnippet}
\`\`\``:""}`).join(`
...
*(Reply with your answer and I will critique your reasoning!)*`;if(i.includes("extension")||i.includes("suggest")||i.includes("more")||i.includes("advance"))return`### 💡 High-Impact Architectural Extensions for **${d.title}**

`+a.suggestedExtensions.map((c,G)=>`**${G+1}. ${c.title}**
- **Implementation**: ${c.description}
- **Architectural Trade-off & Impact**: ${c.architecturalImpact}`).join(`

`)+`

*Which extension would you like to explore or scaffold into code?*`;if(i.includes("defense")||i.includes("interview")||i.includes("question")||i.includes("spar")||i.includes("challenge")){const c=a.interviewDefenseQuestions[0];return`### 🎯 Staff-Level System Defense Challenge for **${d.title}**
...
*(How would you defend your choice if the interviewer asks about trade-offs? Type your response below!)*`}for(const c of d.layers)if(i.includes(c.layer.toLowerCase()))return`### 🏛️ ${c.layer} Layer Deep-Dive: **${d.title}**

- **Key Components**: ${c.components.join(", ")}
- **Core Invariant**: ${c.invariants.join(" ")}

#### Why is this layer decoupled?
In Clean Hexagonal architecture, the **${c.layer} Layer** has strict boundary rules. It does not leak implementation details into surrounding subsystems, allowing us to swap the rendering substrate (e.g. Canvas2D to WebGPU) without altering domain logic.`;return`### 🏛️ Staff Architectural Analysis: **${d.title}**
...
*Would you like me to walk through the implementation code, run a mock defense question, or explore how this satisfies the syllabus?*`}return o==="roadmap"&&(t!=null&&t.topicTitle)?i.includes("teach me")||i.includes("how does it work")||i.includes("explain")||i.includes("breakdown")?`### 🧠 Socratic Deep-Dive: **${t.topicTitle}**
...
${((h=t.keyPoints)==null?void 0:h.map(d=>`- ${d}`).join(`
...
*Would you like to test your intuition with a rapid-fire duel or look up the primary RFC spec?*`:`### 🎓 Socratic Tutor: **${t.topicTitle}**

You asked: *"**${l}**"*

In the **${t.trackName||"Crucible"}** track, **${t.topicTitle}** operates under this core rule:
> **${((S=t.keyPoints)==null?void 0:S[0])||"Enforce deterministic invariants without main-thread blocking."}**
...
*Ask me for an instant duel or a deep code example!*`:o==="sandbox"?n!=null&&n.error?`### 🛠️ Reflexion Error Diagnosis & Repair
...
**Recommended Action**:
Click **"Format"** or ask me to scaffold a clean template!`:`### ⚡ Sandbox Code Copilot
...
*You can ask me to write a custom React 19 component (e.g. with \`useActionState\` or \`useOptimistic\`), optimize rendering, or scaffold a virtualized data table!*`:`### 🔮 Socratic Technical Mentor
...
*How can I help you deepen your preparation?*`},[o,r,t,n]),L=p.useCallback(async l=>{if(!l.trim()||k)return;const i={id:`user-${Date.now()}`,role:"user",content:l.trim(),timestamp:Date.now()};g(s=>[...s,i]),v(!0);try{let s="";if(D&&y){const h=M(l),S=I.filter(a=>a.role==="user"||a.role==="assistant").slice(-6).map(a=>({role:a.role,content:a.content}));s=await y({unitTitle:(t==null?void 0:t.topicTitle)||(r==null?void 0:r.projectTitle)||"AI Session",category:(t==null?void 0:t.area)||"Architecture",trackName:(t==null?void 0:t.trackName)||"Crucible",taskDescription:h,specs:(t==null?void 0:t.keyPoints)||[],userCode:(n==null?void 0:n.jsxCode)||(u==null?void 0:u.userCode)||"",practiceType:"code",messages:[...S,{role:"user",content:h}]})||""}s||(await new Promise(h=>setTimeout(h,450)),s=C(l));const m={id:`assistant-${Date.now()}`,role:"assistant",content:s,timestamp:Date.now(),persona:b};g(h=>[...h,m])}catch(s){console.error("Agent chat error:",s),g(m=>[...m,{id:`error-${Date.now()}`,role:"assistant",content:`⚠️ Failed to complete response: ${(s==null?void 0:s.message)||"Unknown engine error."}`,timestamp:Date.now()}])}finally{v(!1)}},[k,D,y,M,t,r,n,u,I,C,b]),B=p.useCallback(()=>{const l=(t==null?void 0:t.topicTitle)||(r==null?void 0:r.projectTitle)||"React Internals";t!=null&&t.area;const i=he(l),s={id:`duel-${Date.now()}`,role:"tool",toolType:"duel",toolData:i,content:`🎮 **Gamified Battle Generated: ${i.duelTitle}**

Answer the challenge question to prove your architectural intuition!`,timestamp:Date.now(),persona:"duel"};g(m=>[...m,s])},[t,r]),O=p.useCallback(l=>{const i=ge(l),s={id:`lit-${Date.now()}`,role:"tool",toolType:"literature",toolData:i,content:`📚 **Verified Literature & Specification Search for "${l}"**
Found ${i.length} primary reference(s) from React RFCs, W3C, WHATWG, and V8 Team.`,timestamp:Date.now(),persona:"search"};g(m=>[...m,s])},[]),F=p.useCallback(()=>{if(!(r!=null&&r.blueprint))return;const l=P[r.blueprint.id];if(!l)return;let i=`### 💡 High-Impact Architectural Extensions for **${r.blueprint.title}**

`;l.suggestedExtensions.forEach((s,m)=>{i+=`**${m+1}. ${s.title}**
- **Implementation**: ${s.description}
- **Architectural Invariant & Impact**: ${s.architecturalImpact}

`}),g(s=>[...s,{id:`ext-${Date.now()}`,role:"assistant",content:i,timestamp:Date.now(),persona:"architect"}])},[r]),T=p.useCallback(()=>{if(!(r!=null&&r.blueprint))return;const l=P[r.blueprint.id];if(!l)return;const i=`### 📊 Syllabus Coverage Audit: **${r.blueprint.title}**

**Coverage Rating**: **${l.syllabusCoveragePercentage}%** of Senior/Staff frontend interview topics.

**Directly Exercised Syllabus Areas**:
`+l.coveredSyllabusAreas.map(s=>`✅ **${s}**`).join(`
`)+`

**Explicit Topics Matrix**: ${r.blueprint.explicitTopics.length} topics mapped.
**Implicit V8 & Browser Foundations**: ${r.blueprint.implicitFoundations.length} deep invariants.
**Framework vs Manual**: ${r.blueprint.frameworkVsManual.manualEngineeringRequired.length} manual low-level subsystems you build yourself.`;g(s=>[...s,{id:`audit-${Date.now()}`,role:"assistant",content:i,timestamp:Date.now(),persona:"architect",toolType:"syllabus_audit"}])},[r]),N=p.useCallback(l=>{const i=pe[l];i&&(n!=null&&n.onApplyCode&&n.onApplyCode(i.jsx,i.css),g(s=>[...s,{id:`scaffold-${Date.now()}`,role:"tool",toolType:"code_patch",content:`🚀 **Scaffold Applied to Sandbox**: *${i.title}*

Injected production-grade component JSX and styling directly into your scratchpad editor.`,timestamp:Date.now(),persona:"copilot"}]))},[n]),$=p.useCallback(()=>{g([])},[]);return{messages:I,isTyping:k,activePersona:b,setActivePersona:x,sendMessage:L,triggerGamifiedDuel:B,searchLiterature:O,suggestProjectExtensions:F,auditProjectSyllabus:T,scaffoldSandboxTemplate:N,clearMessages:$}}function we({isOpen:o,onClose:t,contextType:r,roadmapContext:n,projectContext:u,sandboxContext:y,masteryContext:D,chatWithMentor:I,isAiReady:g}){const{messages:k,isTyping:v,activePersona:b,setActivePersona:x,sendMessage:w,triggerGamifiedDuel:M,searchLiterature:C,suggestProjectExtensions:L,auditProjectSyllabus:B,scaffoldSandboxTemplate:O,clearMessages:F}=fe({contextType:r,roadmapContext:n,projectContext:u,sandboxContext:y,masteryContext:D,chatWithMentor:I,isAiReady:g}),[T,N]=p.useState(""),[$,l]=p.useState({}),i=p.useRef(null);if(p.useEffect(()=>{var a;(a=i.current)==null||a.scrollIntoView({behavior:"smooth"})},[k,v]),!o)return null;const s=()=>{!T.trim()||v||(w(T),N(""))},m=a=>{a.key==="Enter"&&!a.shiftKey&&(a.preventDefault(),s())},S=(()=>{var a;return r==="roadmap"?{icon:z,title:(n==null?void 0:n.topicTitle)||"Roadmap Track",subtitle:(n==null?void 0:n.trackName)||"Core Curriculum",color:"from-sky-500 to-indigo-500"}:r==="project"?{icon:Y,title:(u==null?void 0:u.projectTitle)||"Tier-1 System Architecture",subtitle:((a=u==null?void 0:u.blueprint)==null?void 0:a.realWorldAnalog)||"Staff Blueprint",color:"from-amber-500 to-orange-500"}:r==="sandbox"?{icon:K,title:"Sandbox Lab & Compiler",subtitle:y!=null&&y.error?"Transpiler Error Active":"React 19 Execution",color:"from-emerald-500 to-teal-500"}:{icon:U,title:"Interview Mastery Oracle",subtitle:"Universal Socratic Substrate",color:"from-purple-500 to-indigo-500"}})(),d=S.icon;return e.jsxs("div",{className:"fixed inset-y-0 right-0 w-full sm:w-[480px] lg:w-[540px] z-50 bg-slate-900/98 backdrop-blur-xl border-l border-slate-800 shadow-2xl flex flex-col text-slate-100 transition-all animate-in slide-in-from-right duration-200",children:[e.jsxs("div",{className:"p-3.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between gap-3 shrink-0",children:[e.jsxs("div",{className:"flex items-center gap-2.5 min-w-0",children:[e.jsx("div",{className:`w-8 h-8 rounded-xl bg-gradient-to-tr ${S.color} flex items-center justify-center shrink-0 shadow-md`,children:e.jsx(d,{size:16,className:"text-white"})}),e.jsxs("div",{className:"min-w-0",children:[e.jsxs("div",{className:"flex items-center gap-1.5",children:[e.jsx("span",{className:"font-black text-xs tracking-tight text-white truncate",children:S.title}),e.jsx("span",{className:`w-2 h-2 rounded-full ${g?"bg-emerald-400 animate-pulse":"bg-amber-400"} shrink-0`})]}),e.jsx("p",{className:"text-[10px] text-slate-400 font-mono truncate",children:S.subtitle})]})]}),e.jsxs("div",{className:"flex items-center gap-1 shrink-0",children:[e.jsx("button",{onClick:F,title:"Reset Chat Session",className:"p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition cursor-pointer",children:e.jsx(ae,{size:14})}),e.jsx("button",{onClick:t,title:"Close Assistant",className:"p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition cursor-pointer",children:e.jsx(ne,{size:16})})]})]}),e.jsx("div",{className:"p-2 bg-slate-950/60 border-b border-slate-800 flex items-center gap-1 overflow-x-auto no-scrollbar shrink-0",children:[{id:"tutor",label:"🎓 Socratic Tutor",icon:z},{id:"architect",label:"🏛️ Systems Architect",icon:q},{id:"copilot",label:"🛠️ Code Copilot",icon:Z},{id:"duel",label:"🎮 Gamified Duel",icon:J},{id:"search",label:"📚 Literature / RFCs",icon:_}].map(a=>{const c=b===a.id;return e.jsx("button",{onClick:()=>x(a.id),className:`px-2.5 py-1 rounded-lg text-[11px] font-bold whitespace-nowrap transition cursor-pointer flex items-center gap-1 border ${c?"bg-sky-600 text-white border-sky-400 shadow-xs":"bg-slate-900/80 text-slate-400 border-slate-800 hover:text-slate-200 hover:bg-slate-800"}`,children:e.jsx("span",{children:a.label})},a.id)})}),e.jsxs("div",{className:"p-2 bg-slate-950/40 border-b border-slate-800/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0 text-[11px]",children:[r==="roadmap"&&e.jsxs(e.Fragment,{children:[e.jsxs("button",{onClick:M,className:"px-2.5 py-1 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(J,{size:11})," ",e.jsx("span",{children:"⚡ Instant Duel"})]}),e.jsxs("button",{onClick:()=>C((n==null?void 0:n.topicTitle)||"React 19"),className:"px-2.5 py-1 rounded-lg bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(_,{size:11})," ",e.jsx("span",{children:"📚 Curated RFCs"})]}),e.jsx("button",{onClick:()=>w(`What is the most common memory leak or V8 deoptimization trap in ${n==null?void 0:n.topicTitle}?`),className:"px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition shrink-0",children:e.jsx("span",{children:"⚠️ V8 Trap"})})]}),r==="project"&&e.jsxs(e.Fragment,{children:[e.jsxs("button",{onClick:L,className:"px-2.5 py-1 rounded-lg bg-indigo-500/15 hover:bg-indigo-500/25 border border-indigo-500/30 text-indigo-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(U,{size:11})," ",e.jsx("span",{children:"💡 Extensions"})]}),e.jsxs("button",{onClick:B,className:"px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(Q,{size:11})," ",e.jsx("span",{children:"📊 Syllabus Audit"})]}),e.jsxs("button",{onClick:()=>{var a;return w(`Run a Staff-level interview defense challenge against my architecture for ${(a=u==null?void 0:u.blueprint)==null?void 0:a.title}. Catch any hand-waving!`)},className:"px-2.5 py-1 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(Y,{size:11})," ",e.jsx("span",{children:"🎯 Mock Defense"})]})]}),r==="sandbox"&&e.jsxs(e.Fragment,{children:[(y==null?void 0:y.error)&&e.jsxs("button",{onClick:()=>w(`Debug and fix this compiler error: ${y.error}`),className:"px-2.5 py-1 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(Z,{size:11})," ",e.jsx("span",{children:"🛠️ Fix Compiler Error"})]}),e.jsxs("button",{onClick:()=>O("counter-advanced"),className:"px-2.5 py-1 rounded-lg bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(K,{size:11})," ",e.jsx("span",{children:"🚀 Insert React 19 State"})]}),e.jsxs("button",{onClick:()=>O("virtual-list"),className:"px-2.5 py-1 rounded-lg bg-purple-500/15 hover:bg-purple-500/25 border border-purple-500/30 text-purple-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(q,{size:11})," ",e.jsx("span",{children:"📦 Virtual List Scaffold"})]})]})]}),e.jsxs("div",{className:"flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar",children:[k.map(a=>{var H;const c=a.role==="user",G=a.role==="tool";return e.jsxs("div",{className:`flex items-start gap-2.5 ${c?"justify-end":"justify-start"}`,children:[!c&&e.jsx("div",{className:"w-6 h-6 rounded-lg bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center shrink-0 mt-0.5 text-indigo-400",children:e.jsx(se,{size:13})}),e.jsx("div",{className:`max-w-[92%] p-3.5 rounded-2xl text-xs ${c?"bg-sky-600 text-white rounded-tr-xs shadow-md":G?"bg-slate-950 border border-indigo-500/30 rounded-tl-xs shadow-md w-full":"bg-slate-800/95 border border-slate-700/80 rounded-tl-xs shadow-md"}`,children:c?e.jsx("p",{className:"whitespace-pre-wrap leading-relaxed",children:a.content}):e.jsxs("div",{className:"space-y-3",children:[e.jsx(ie,{text:a.content}),a.toolType==="duel"&&a.toolData&&e.jsxs("div",{className:"mt-3 p-3.5 rounded-xl bg-slate-900 border border-amber-500/40 space-y-3",children:[e.jsxs("div",{className:"flex items-center justify-between gap-2",children:[e.jsx("span",{className:"text-amber-300 font-bold font-mono text-[11px]",children:a.toolData.duelTitle}),e.jsx("span",{className:"px-2 py-0.5 rounded text-[9px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40",children:a.toolData.badge})]}),(H=a.toolData.questions)==null?void 0:H.map((f,E)=>{const j=$[`${a.id}-${E}`],W=j!==void 0,V=j===f.correctIndex;return e.jsxs("div",{className:"space-y-2 pt-2 border-t border-slate-800",children:[e.jsxs("p",{className:"font-medium text-slate-200",children:[E+1,". ",f.question]}),e.jsx("div",{className:"space-y-1.5",children:f.options.map((ee,A)=>{const te=j===A;return e.jsxs("button",{disabled:W,onClick:()=>l(re=>({...re,[`${a.id}-${E}`]:A})),className:`w-full text-left p-2 rounded-lg text-[11px] transition cursor-pointer border flex items-center justify-between gap-2 ${W?A===f.correctIndex?"bg-emerald-950/80 text-emerald-200 border-emerald-500/60":te?"bg-rose-950/80 text-rose-200 border-rose-500/60":"bg-slate-950 text-slate-400 border-slate-800 opacity-60":"bg-slate-950 text-slate-300 border-slate-800 hover:border-sky-500/50 hover:bg-slate-800"}`,children:[e.jsx("span",{children:ee}),W&&A===f.correctIndex&&e.jsx(Q,{size:12,className:"text-emerald-400 shrink-0"})]},A)})}),W&&e.jsxs("div",{className:`p-2 rounded-lg text-[11px] ${V?"bg-emerald-950/60 text-emerald-300 border border-emerald-800/60":"bg-rose-950/60 text-rose-300 border border-rose-800/60"}`,children:[e.jsx("p",{className:"font-bold",children:V?"🎯 Correct!":"❌ Invariant Violation"}),e.jsx("p",{className:"text-slate-300 mt-1",children:f.explanation})]})]},E)})]}),a.toolType==="literature"&&Array.isArray(a.toolData)&&e.jsx("div",{className:"mt-3 space-y-2",children:a.toolData.map(f=>e.jsxs("div",{className:"p-3 rounded-xl bg-slate-950 border border-sky-500/30 space-y-1.5",children:[e.jsxs("div",{className:"flex items-center justify-between gap-2",children:[e.jsx("span",{className:"font-bold text-sky-300 text-xs",children:f.title}),e.jsx("span",{className:"px-1.5 py-0.5 rounded text-[9px] font-mono bg-sky-950 text-sky-400 border border-sky-800",children:f.category})]}),e.jsx("p",{className:"text-slate-300 text-[11px] leading-relaxed",children:f.summary}),e.jsxs("div",{className:"pt-1 flex items-center justify-between gap-2 text-[10px]",children:[e.jsx("span",{className:"text-slate-500",children:f.authorOrOrg}),e.jsxs("a",{href:f.url,target:"_blank",rel:"noreferrer",className:"text-sky-400 hover:text-sky-300 flex items-center gap-1 font-bold",children:[e.jsx("span",{children:"Read Spec"}),e.jsx(oe,{size:10})]})]})]},f.id))})]})})]},a.id)}),v&&e.jsxs("div",{className:"flex items-center gap-2.5 text-slate-400 text-xs pl-1",children:[e.jsx("div",{className:"w-6 h-6 rounded-lg bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center shrink-0 text-indigo-400",children:e.jsx(U,{size:13,className:"animate-spin"})}),e.jsxs("div",{className:"flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/60 px-3 py-2 rounded-xl text-[11px] text-indigo-300",children:[e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce"}),e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce [animation-delay:0.2s]"}),e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce [animation-delay:0.4s]"}),e.jsx("span",{className:"ml-1 text-slate-400 font-mono",children:"Running Chain-of-Verification (CoVe)..."})]})]}),e.jsx("div",{ref:i})]}),e.jsxs("div",{className:"p-3 bg-slate-950 border-t border-slate-800 shrink-0 flex items-end gap-2",children:[e.jsx("textarea",{value:T,onChange:a=>N(a.target.value),onKeyDown:m,placeholder:`Ask the ${b} anything... (Enter to send, Shift+Enter for newline)`,rows:2,className:"flex-1 bg-slate-900 border border-slate-700/70 text-slate-100 text-xs rounded-xl px-3.5 py-2.5 outline-none focus:ring-2 focus:ring-sky-500/40 focus:border-sky-500/50 resize-none max-h-28 transition placeholder:text-slate-500 custom-scrollbar"}),e.jsx("button",{onClick:s,disabled:!T.trim()||v,className:"p-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 disabled:opacity-40 text-white transition cursor-pointer shadow-md shrink-0 mb-0.5",children:e.jsx(ce,{size:15})})]})]})}export{R as P,we as U};
