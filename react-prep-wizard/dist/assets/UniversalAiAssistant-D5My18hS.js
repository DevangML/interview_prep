...
.metric { font-size: 11px; font-family: monospace; color: #38bdf8; }`}};function Se(i){if(!i.trim())return X;const t=i.toLowerCase();return X.filter(r=>r.title.toLowerCase().includes(t)||r.summary.toLowerCase().includes(t)||r.category.toLowerCase().includes(t)||r.applicableTopics.some(n=>n.toLowerCase().includes(t))||r.keyInvariants.some(n=>n.toLowerCase().includes(t)))}function ve(i,t){return{duelTitle:`⚡ Rapid Clash: ${i}`,badge:"Real-Time Dialectic",questions:[{question:`In a high-throughput React system, what is the primary invariant for "${i}"?`,options:["It guarantees O(1) time complexity by caching all intermediate states in localStorage.","It enforces deterministic state transformations while avoiding main-thread Long Animation Frames (LoAF).","It disables the Fiber reconciler tree diffing algorithm completely.","It replaces the browser microtask queue with a native synchronous thread."],correctIndex:1,explanation:`Under the hood, "${i}" aligns with the React 19 concurrent runtime by batching state changes and preserving main-thread responsiveness.`},{question:`When debugging memory leaks related to "${i}", what should you inspect in Chrome DevTools?`,options:["Retainer trees in Heap Snapshots for un-cleaned event listeners and detached DOM nodes.","The CSS style rules panel for unused classes.","The network tab status code for 200 OK headers.","The cookie expiration date timestamps."],correctIndex:0,explanation:"Heap Snapshot retainers reveal root references preventing the V8 Garbage Collector from reclaiming memory."}]}}function we({contextType:i,roadmapContext:t,projectContext:r,sandboxContext:n,masteryContext:d,chatWithMentor:b,isAiReady:M}){const[A,h]=u.useState([]),[k,v]=u.useState(!1),[f,W]=u.useState(()=>i==="project"?"architect":i==="sandbox"?"copilot":"tutor"),w=u.useRef("");u.useEffect(()=>{const l=`${i}:${(t==null?void 0:t.topicId)||(r==null?void 0:r.projectId)||"main"}`;if(w.current!==l){w.current=l;let s="";if(i==="roadmap"&&(t!=null&&t.topicTitle))s=`👋 **Welcome to the ${t.topicTitle} Socratic Hub!**
...
3. **Concurrent Reconciliation**: Interruptible render phases and deterministic commit phases.`;const S={id:`assistant-${Date.now()}`,role:"assistant",content:c,timestamp:Date.now(),persona:f};h(p=>[...p,S])}catch(c){console.error("Agent chat error:",c),h(S=>[...S,{id:`error-${Date.now()}`,role:"assistant",content:`⚠️ Failed to complete response: ${(c==null?void 0:c.message)||"Unknown engine error."}`,timestamp:Date.now()}])}finally{v(!1)}},[k,M,b,C,i,f,t,r,n,d,A]),B=u.useCallback(()=>{const l=(t==null?void 0:t.topicTitle)||(r==null?void 0:r.projectTitle)||"React Internals";t!=null&&t.area;const s=ve(l),o={id:`duel-${Date.now()}`,role:"tool",toolType:"duel",toolData:s,content:`🎮 **Gamified Battle Generated: ${s.duelTitle}**

Answer the challenge question to prove your architectural intuition!`,timestamp:Date.now(),persona:"duel"};h(c=>[...c,o])},[t,r]),F=u.useCallback(l=>{const s=Se(l),o={id:`lit-${Date.now()}`,role:"tool",toolType:"literature",toolData:s,content:`📚 **Verified Literature & Specification Search for "${l}"**
...
`}),h(o=>[...o,{id:`ext-${Date.now()}`,role:"assistant",content:s,timestamp:Date.now(),persona:"architect"}])},[r]),G=u.useCallback(()=>{if(!(r!=null&&r.blueprint))return;const l=R[r.blueprint.id];if(!l)return;const s=`### 📊 Syllabus Coverage Audit: ${r.blueprint.title}
...
**Implicit V8 & Browser Foundations**: ${r.blueprint.implicitFoundations.length} deep invariants.`;h(o=>[...o,{id:`audit-${Date.now()}`,role:"assistant",content:s,timestamp:Date.now(),persona:"architect",toolType:"syllabus_audit"}])},[r]),T=u.useCallback(l=>{const s=be[l];s&&(n!=null&&n.onApplyCode&&n.onApplyCode(s.jsx,s.css),h(o=>[...o,{id:`scaffold-${Date.now()}`,role:"tool",toolType:"code_patch",content:`🚀 **Scaffold Applied to Sandbox**: *${s.title}*

Injected production-grade component JSX and styling directly into your scratchpad editor.`,timestamp:Date.now(),persona:"copilot"}]))},[n]),E=u.useCallback(()=>{h([])},[]);return{messages:A,isTyping:k,activePersona:f,setActivePersona:W,sendMessage:L,triggerGamifiedDuel:B,searchLiterature:F,suggestProjectExtensions:D,auditProjectSyllabus:G,scaffoldSandboxTemplate:T,clearMessages:E}}function Pe({isOpen:i,onClose:t,contextType:r,roadmapContext:n,projectContext:d,sandboxContext:b,masteryContext:M,chatWithMentor:A,isAiReady:h}){const{messages:k,isTyping:v,activePersona:f,setActivePersona:W,sendMessage:w,triggerGamifiedDuel:C,searchLiterature:L,suggestProjectExtensions:B,auditProjectSyllabus:F,scaffoldSandboxTemplate:D,clearMessages:G}=we({contextType:r,roadmapContext:n,projectContext:d,sandboxContext:b,masteryContext:M,chatWithMentor:A,isAiReady:h}),[T,E]=u.useState(""),[l,s]=u.useState({}),o=u.useRef(null);if(u.useEffect(()=>{var a;(a=o.current)==null||a.scrollIntoView({behavior:"smooth"})},[k,v]),!i)return null;const c=()=>{!T.trim()||v||(w(T),E(""))},S=a=>{a.key==="Enter"&&!a.shiftKey&&(a.preventDefault(),c())},m=(()=>{var a;return r==="roadmap"?{icon:V,title:(n==null?void 0:n.topicTitle)||"Roadmap Track",subtitle:(n==null?void 0:n.trackName)||"Core Curriculum",color:"from-sky-500 to-indigo-500"}:r==="project"?{icon:Z,title:(d==null?void 0:d.projectTitle)||"Tier-1 System Architecture",subtitle:((a=d==null?void 0:d.blueprint)==null?void 0:a.realWorldAnalog)||"Staff Blueprint",color:"from-amber-500 to-orange-500"}:r==="sandbox"?{icon:K,title:"Sandbox Lab & Compiler",subtitle:b!=null&&b.error?"Transpiler Error Active":"React 19 Execution",color:"from-emerald-500 to-teal-500"}:{icon:H,title:"Interview Mastery Oracle",subtitle:"Universal Socratic Substrate",color:"from-purple-500 to-indigo-500"}})(),g=m.icon;return e.jsxs("div",{className:"fixed inset-y-0 right-0 w-full sm:w-[480px] lg:w-[540px] z-50 bg-slate-900/98 backdrop-blur-xl border-l border-slate-800 shadow-2xl flex flex-col text-slate-100 transition-all animate-in slide-in-from-right duration-200",children:[e.jsxs("div",{className:"p-3.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between gap-3 shrink-0",children:[e.jsxs("div",{className:"flex items-center gap-2.5 min-w-0",children:[e.jsx("div",{className:`w-8 h-8 rounded-xl bg-gradient-to-tr ${m.color} flex items-center justify-center shrink-0 shadow-md`,children:e.jsx(g,{size:16,className:"text-white"})}),e.jsxs("div",{className:"min-w-0",children:[e.jsxs("div",{className:"flex items-center gap-1.5",children:[e.jsx("span",{className:"font-black text-xs tracking-tight text-white truncate",children:m.title}),e.jsx("span",{className:`w-2 h-2 rounded-full ${h?"bg-emerald-400 animate-pulse":"bg-amber-400"} shrink-0`})]}),e.jsx("p",{className:"text-[10px] text-slate-400 font-mono truncate",children:m.subtitle})]})]}),e.jsxs("div",{className:"flex items-center gap-1 shrink-0",children:[e.jsx("button",{onClick:G,title:"Reset Chat Session",className:"p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition cursor-pointer",children:e.jsx(le,{size:14})}),e.jsx("button",{onClick:t,title:"Close Assistant",className:"p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition cursor-pointer",children:e.jsx(de,{size:16})})]})]}),e.jsx("div",{className:"p-2 bg-slate-950/60 border-b border-slate-800 flex items-center gap-1 overflow-x-auto no-scrollbar shrink-0",children:[{id:"tutor",label:"🎓 Socratic Tutor",icon:V},{id:"architect",label:"🏛️ Systems Architect",icon:$},{id:"copilot",label:"🛠️ Code Copilot",icon:q},{id:"duel",label:"🎮 Gamified Duel",icon:_},{id:"search",label:"📚 Literature / RFCs",icon:J}].map(a=>{const I=f===a.id;return e.jsx("button",{onClick:()=>W(a.id),className:`px-2.5 py-1 rounded-lg text-[11px] font-bold whitespace-nowrap transition cursor-pointer flex items-center gap-1 border ${I?"bg-sky-600 text-white border-sky-400 shadow-xs":"bg-slate-900/80 text-slate-400 border-slate-800 hover:text-slate-200 hover:bg-slate-800"}`,children:e.jsx("span",{children:a.label})},a.id)})}),e.jsxs("div",{className:"p-2 bg-slate-950/40 border-b border-slate-800/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0 text-[11px]",children:[r==="roadmap"&&e.jsxs(e.Fragment,{children:[e.jsxs("button",{onClick:C,className:"px-2.5 py-1 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(_,{size:11})," ",e.jsx("span",{children:"⚡ Instant Duel"})]}),e.jsxs("button",{onClick:()=>L((n==null?void 0:n.topicTitle)||"React 19"),className:"px-2.5 py-1 rounded-lg bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(J,{size:11})," ",e.jsx("span",{children:"📚 Curated RFCs"})]}),e.jsx("button",{onClick:()=>w(`What is the most common memory leak or V8 deoptimization trap in ${n==null?void 0:n.topicTitle}?`),className:"px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition shrink-0",children:e.jsx("span",{children:"⚠️ V8 Trap"})})]}),r==="project"&&e.jsxs(e.Fragment,{children:[e.jsxs("button",{onClick:B,className:"px-2.5 py-1 rounded-lg bg-indigo-500/15 hover:bg-indigo-500/25 border border-indigo-500/30 text-indigo-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(H,{size:11})," ",e.jsx("span",{children:"💡 Extensions"})]}),e.jsxs("button",{onClick:F,className:"px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(Q,{size:11})," ",e.jsx("span",{children:"📊 Syllabus Audit"})]}),e.jsxs("button",{onClick:()=>{var a;return w(`Run a Staff-level interview defense challenge against my architecture for ${(a=d==null?void 0:d.blueprint)==null?void 0:a.title}. Catch any hand-waving!`)},className:"px-2.5 py-1 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(Z,{size:11})," ",e.jsx("span",{children:"🎯 Mock Defense"})]})]}),r==="sandbox"&&e.jsxs(e.Fragment,{children:[(b==null?void 0:b.error)&&e.jsxs("button",{onClick:()=>w(`Debug and fix this compiler error: ${b.error}`),className:"px-2.5 py-1 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(q,{size:11})," ",e.jsx("span",{children:"🛠️ Fix Compiler Error"})]}),e.jsxs("button",{onClick:()=>D("counter-advanced"),className:"px-2.5 py-1 rounded-lg bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx(K,{size:11})," ",e.jsx("span",{children:"🚀 Insert React 19 State"})]}),e.jsxs("button",{onClick:()=>D("virtual-list"),className:"px-2.5 py-1 rounded-lg bg-purple-500/15 hover:bg-purple-500/25 border border-purple-500/30 text-purple-300 font-bold transition flex items-center gap-1 shrink-0",children:[e.jsx($,{size:11})," ",e.jsx("span",{children:"📦 Virtual List Scaffold"})]})]})]}),e.jsxs("div",{className:"flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar",children:[k.map(a=>{var j;const I=a.role==="user",Y=a.role==="tool";return e.jsxs("div",{className:`flex items-start gap-2.5 ${I?"justify-end":"justify-start"}`,children:[!I&&e.jsx("div",{className:"w-6 h-6 rounded-lg bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center shrink-0 mt-0.5 text-indigo-400",children:e.jsx(ue,{size:13})}),e.jsx("div",{className:`max-w-[92%] p-3.5 rounded-2xl text-xs ${I?"bg-sky-600 text-white rounded-tr-xs shadow-md":Y?"bg-slate-950 border border-indigo-500/30 rounded-tl-xs shadow-md w-full":"bg-slate-800/95 border border-slate-700/80 rounded-tl-xs shadow-md"}`,children:I?e.jsx("p",{className:"whitespace-pre-wrap leading-relaxed",children:a.content}):e.jsxs("div",{className:"space-y-3",children:[e.jsx(ce,{text:a.content}),a.toolType==="duel"&&a.toolData&&e.jsxs("div",{className:"mt-3 p-3.5 rounded-xl bg-slate-900 border border-amber-500/40 space-y-3",children:[e.jsxs("div",{className:"flex items-center justify-between gap-2",children:[e.jsx("span",{className:"text-amber-300 font-bold font-mono text-[11px]",children:a.toolData.duelTitle}),e.jsx("span",{className:"px-2 py-0.5 rounded text-[9px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40",children:a.toolData.badge})]}),(j=a.toolData.questions)==null?void 0:j.map((y,N)=>{const U=l[`${a.id}-${N}`],x=U!==void 0,z=U===y.correctIndex;return e.jsxs("div",{className:"space-y-2 pt-2 border-t border-slate-800",children:[e.jsxs("p",{className:"font-medium text-slate-200",children:[N+1,". ",y.question]}),e.jsx("div",{className:"space-y-1.5",children:y.options.map((ee,P)=>{const re=U===P;return e.jsxs("button",{disabled:x,onClick:()=>s(te=>({...te,[`${a.id}-${N}`]:P})),className:`w-full text-left p-2 rounded-lg text-[11px] transition cursor-pointer border flex items-center justify-between gap-2 ${x?P===y.correctIndex?"bg-emerald-950/80 text-emerald-200 border-emerald-500/60":re?"bg-rose-950/80 text-rose-200 border-rose-500/60":"bg-slate-950 text-slate-400 border-slate-800 opacity-60":"bg-slate-950 text-slate-300 border-slate-800 hover:border-sky-500/50 hover:bg-slate-800"}`,children:[e.jsx("span",{children:ee}),x&&P===y.correctIndex&&e.jsx(Q,{size:12,className:"text-emerald-400 shrink-0"})]},P)})}),x&&e.jsxs("div",{className:`p-2 rounded-lg text-[11px] ${z?"bg-emerald-950/60 text-emerald-300 border border-emerald-800/60":"bg-rose-950/60 text-rose-300 border border-rose-800/60"}`,children:[e.jsx("p",{className:"font-bold",children:z?"🎯 Correct!":"❌ Invariant Violation"}),e.jsx("p",{className:"text-slate-300 mt-1",children:y.explanation})]})]},N)})]}),a.toolType==="literature"&&Array.isArray(a.toolData)&&e.jsx("div",{className:"mt-3 space-y-2",children:a.toolData.map(y=>e.jsxs("div",{className:"p-3 rounded-xl bg-slate-950 border border-sky-500/30 space-y-1.5",children:[e.jsxs("div",{className:"flex items-center justify-between gap-2",children:[e.jsx("span",{className:"font-bold text-sky-300 text-xs",children:y.title}),e.jsx("span",{className:"px-1.5 py-0.5 rounded text-[9px] font-mono bg-sky-950 text-sky-400 border border-sky-800",children:y.category})]}),e.jsx("p",{className:"text-slate-300 text-[11px] leading-relaxed",children:y.summary}),e.jsxs("div",{className:"pt-1 flex items-center justify-between gap-2 text-[10px]",children:[e.jsx("span",{className:"text-slate-500",children:y.authorOrOrg}),e.jsxs("a",{href:y.url,target:"_blank",rel:"noreferrer",className:"text-sky-400 hover:text-sky-300 flex items-center gap-1 font-bold",children:[e.jsx("span",{children:"Read Spec"}),e.jsx(pe,{size:10})]})]})]},y.id))})]})})]},a.id)}),v&&e.jsxs("div",{className:"flex items-center gap-2.5 text-slate-400 text-xs pl-1",children:[e.jsx("div",{className:"w-6 h-6 rounded-lg bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center shrink-0 text-indigo-400",children:e.jsx(H,{size:13,className:"animate-spin"})}),e.jsxs("div",{className:"flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/60 px-3 py-2 rounded-xl text-[11px] text-indigo-300",children:[e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce"}),e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce [animation-delay:0.2s]"}),e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-indigo-400 animate-bounce [animation-delay:0.4s]"}),e.jsx("span",{className:"ml-1 text-slate-400 font-mono",children:"Running Chain-of-Verification (CoVe)..."})]})]}),e.jsx("div",{ref:o})]}),e.jsxs("div",{className:"p-3 bg-slate-950 border-t border-slate-800 shrink-0 flex items-end gap-2",children:[e.jsx("textarea",{value:T,onChange:a=>E(a.target.value),onKeyDown:S,placeholder:`Ask the ${f} anything... (Enter to send, Shift+Enter for newline)`,rows:2,className:"flex-1 bg-slate-900 border border-slate-700/70 text-slate-100 text-xs rounded-xl px-3.5 py-2.5 outline-none focus:ring-2 focus:ring-sky-500/40 focus:border-sky-500/50 resize-none max-h-28 transition placeholder:text-slate-500 custom-scrollbar"}),e.jsx("button",{onClick:c,disabled:!T.trim()||v,className:"p-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 disabled:opacity-40 text-white transition cursor-pointer shadow-md shrink-0 mb-0.5",children:e.jsx(me,{size:15})})]})]})}export{O as P,Pe as U};
