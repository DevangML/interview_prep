...
5. LINE ANCHORING:
   ...
   - If no genuine defect exists, emit an empty findings array [].`,K=`You are the Presiding Chief Technical Arbitrator and Court of Appeal for Technical Code Assessments.
...
3. CONSTRUCTIVE & ACTIONABLE: Provide high-leverage mental models, memory lifecycle diagrams, and architectural insights with clean markdown formatting.`,ee="Qwen2.5-Coder-1.5B-Instruct-q4f16_1-MLC";let S=null,U=null,L=null,A=!1,N=!1,P=null,V=0,w=null,_=ee;const $=new Set;function b(){$.forEach(r=>r())}function oe(){const[r,e]=l.useState(L),[i,E]=l.useState(!0),[s,g]=l.useState(A),[x,T]=l.useState(N),[v,y]=l.useState(P),[d,a]=l.useState(V),[f,u]=l.useState(!1),[m,C]=l.useState(w),[D,I]=l.useState(_);l.useEffect(()=>{const t=()=>{e(L),g(A),T(N),y(P),a(V),C(w),I(_)};return $.add(t),t(),L||X().then(o=>{L=o,e(o),!o.webGpuSupported&&typeof navigator<"u"&&!("gpu"in navigator)&&E(!1),b()}),()=>{$.delete(t)}},[]);const j=l.useCallback(async t=>{if(A||N)return;N=!0,w=null,b();const o=typeof t=="string"&&t.trim().length>0?t.trim():_;try{U||(U=new Worker(new URL("/assets/socraticAiWorker-Dc1cCqhL.js",import.meta.url),{type:"module"}));const{CreateWebWorkerMLCEngine:c}=await W(async()=>{const{CreateWebWorkerMLCEngine:p}=await import("./vendor-web-llm-DT0Ab8E6.js");return{CreateWebWorkerMLCEngine:p}},[]);S=await c(U,o,{initProgressCallback:p=>{P=p.text,p.progress!==void 0&&(V=Math.round(p.progress*100)),b()}}),_=o,A=!0,N=!1,P=null,b()}catch(c){console.error("Socratic AI initialization failed:",c),w=(c==null?void 0:c.message)||"Failed to initialize in-browser AI engine.",N=!1,b()}},[]),R=l.useCallback(async t=>{var o,c;if(!A||!S)return null;u(!0);try{const h=`[EXERCISE CONTEXT]
Title: ${t.unitTitle}
Practice Type: ${t.practiceType||"code"}
Task: ${t.taskDescription}
Requirements:
${t.specs.map((M,B)=>`  ${B+1}. ${M}`).join(`
`)}
...
Perform an impartial comparative adjudication. Output strict JSON.`,k=(c=(o=(await S.chat.completions.create({messages:[{role:"system",content:Q},{role:"user",content:h}],response_format:{type:"json_object",schema:JSON.stringify(q)},temperature:0,top_p:1,max_tokens:1200})).choices[0])==null?void 0:o.message)==null?void 0:c.content;if(!k)throw new Error("Empty AI response");return JSON.parse(k)}catch(h){return console.error("Socratic AI Evaluation failed:",h),null}finally{u(!1)}},[]),O=l.useCallback(async t=>{var o,c,h;if(!A||!S)return null;u(!0);try{const p=`[APPEAL CASE CONTEXT]
Problem: ${t.unitTitle}
Task: ${t.taskDescription}
...
Conduct an appellate review of the argument. Output strict JSON.`,M=(h=(c=(await S.chat.completions.create({messages:[{role:"system",content:K},{role:"user",content:p}],response_format:{type:"json_object",schema:JSON.stringify(q)},temperature:0,top_p:1,max_tokens:1200})).choices[0])==null?void 0:c.message)==null?void 0:h.content;if(!M)throw new Error("Empty AI response");return JSON.parse(M)}catch(p){return console.error("Appellate dispute evaluation failed:",p),null}finally{u(!1)}},[]),F=l.useCallback(async t=>{var o,c;if(!A||!S)return null;try{return((c=(o=(await S.chat.completions.create({messages:[{role:"system",content:Z},...t.messages.map(p=>({role:p.role,content:p.content}))],temperature:.3,max_tokens:1024})).choices[0])==null?void 0:o.message)==null?void 0:c.content)||null}catch(h){return console.error("Socratic Chat completion failed:",h),null}},[]);return{hardwareProfile:r,isSupported:i,isReady:s,isLoading:x,downloadProgress:v,progressPercent:d,isAnalyzing:f,error:m,activeModelId:D,initializeEngine:j,evaluateFailure:R,disputeEvaluation:O,chatWithMentor:F}}function te(r){return r.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function H(r){let e=te(r);return e=e.replace(/\*\*\*(.*?)\*\*\*/g,'<strong class="text-amber-300 font-extrabold">$1</strong>'),e=e.replace(/\*\*(.*?)\*\*/g,'<strong class="text-white font-bold">$1</strong>'),e=e.replace(/\*(.*?)\*/g,'<em class="text-sky-300 italic">$1</em>'),e=e.replace(/`([^`]+)`/g,'<code class="px-1.5 py-0.5 rounded-md bg-slate-950 border border-slate-700/80 text-sky-300 font-mono text-[11px] select-all">$1</code>'),e}function ce(r){const e=G.c(11),{text:i,className:E}=r,s=E===void 0?"":E,[g,x]=l.useState(null);let T;e[0]===Symbol.for("react.memo_cache_sentinel")?(T=(f,u)=>{navigator.clipboard.writeText(f),x(u),setTimeout(()=>x(null),2e3)},e[0]=T):T=e[0];const v=T;let y,d;if(e[1]!==s||e[2]!==g||e[3]!==i){const f=i.split(/(```[\s\S]*?```)/g);y=`space-y-3 leading-relaxed text-slate-300 text-xs sm:text-[13px] ${s}`;let u;e[6]!==g?(u=(m,C)=>{if(m.startsWith("```")&&m.endsWith("```")){const I=m.indexOf(`
`),j=I!==-1?m.slice(3,I).trim():"",R=I!==-1?m.slice(I+1,-3):m.slice(3,-3),O=g===C;return n.jsxs("div",{className:"my-3 rounded-xl overflow-hidden border border-slate-800 bg-slate-950 font-mono text-xs shadow-md",children:[n.jsxs("div",{className:"flex items-center justify-between px-3.5 py-1.5 bg-slate-900 border-b border-slate-800 text-[10px] text-slate-400 font-bold uppercase tracking-wider",children:[n.jsx("span",{className:"text-sky-400",children:j||"code"}),n.jsxs("button",{onClick:()=>v(R,C),className:"flex items-center gap-1.5 hover:text-slate-200 cursor-pointer transition text-slate-400",children:[O?n.jsx(J,{size:12,className:"text-emerald-400"}):n.jsx(Y,{size:12}),n.jsx("span",{children:O?"Copied":"Copy"})]})]}),n.jsx("pre",{className:"p-4 overflow-x-auto text-sky-200 leading-relaxed custom-scrollbar font-mono text-xs whitespace-pre-wrap",children:R})]},C)}const D=m.split(`
`);return n.jsx("div",{className:"space-y-2",children:D.map(ie)},C)},e[6]=g,e[7]=u):u=e[7],d=f.map(u),e[1]=s,e[2]=g,e[3]=i,e[4]=y,e[5]=d}else y=e[4],d=e[5];let a;return e[8]!==y||e[9]!==d?(a=n.jsx("div",{className:y,children:d}),e[8]=y,e[9]=d,e[10]=a):a=e[10],a}function ie(r,e){const i=r.trim();return i?i.startsWith("# ")?n.jsx("h2",{className:"text-xl sm:text-2xl font-black text-white mt-4 mb-2 tracking-tight",children:i.slice(2)},e):i.startsWith("## ")?n.jsx("h3",{className:"text-base sm:text-lg font-extrabold text-sky-300 mt-3 mb-1.5 tracking-tight",children:i.slice(3)},e):i.startsWith("### ")?n.jsx("h4",{className:"text-sm font-bold text-amber-300 mt-2.5 mb-1",children:i.slice(4)},e):i.startsWith("> ")?n.jsxs("div",{className:"p-3 my-2 rounded-xl bg-sky-950/30 border-l-4 border-sky-500 text-sky-200 text-xs flex items-start gap-2",children:[n.jsx(z,{size:14,className:"text-sky-400 mt-0.5 shrink-0"}),n.jsx("div",{dangerouslySetInnerHTML:{__html:H(i.slice(2))}})]},e):i.startsWith("- ")||i.startsWith("* ")?n.jsxs("div",{className:"flex items-start gap-2 pl-2",children:[n.jsx("span",{className:"text-sky-400 font-bold select-none text-xs mt-0.5",children:"▪"}),n.jsx("span",{className:"flex-1",dangerouslySetInnerHTML:{__html:H(i.slice(2))}})]},e):n.jsx("p",{className:"leading-relaxed",dangerouslySetInnerHTML:{__html:H(r)}},e):null}export{ce as F,H as f,oe as u};
