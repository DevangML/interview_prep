...
console.log(sum(2, 2));`,specs:["Should use ES6 static imports/exports for tree-shaking support."]}}],Ie=(t,e,n,i,o)=>`/* ${t}
...
`}function tr(){const[t,e]=P.useState(()=>localStorage.getItem("mastery:activeUnit")||ie[0].id),[n,i]=P.useState(()=>{var k;const c=localStorage.getItem("mastery:activeUnit")||ie[0].id,g=localStorage.getItem("mastery:code:"+c);if(g)return g;const S=ze.get(c)??0;return((k=ie[S])==null?void 0:k.practice.starterCode)||ie[0].practice.starterCode}),[o,d]=P.useState(()=>localStorage.getItem("mastery:activeTab")||"editor"),[m,f]=P.useState(""),[b,p]=P.useState(null),[l,N]=P.useState([]),[O,L]=P.useState(()=>{try{return JSON.parse(localStorage.getItem("mastery:solved")||"{}")}catch{return{}}}),{compile:D}=lt(),{formatCSS:I,formatJSX:T}=dt(),[A,_]=P.useState(()=>localStorage.getItem("mastery:portalOpen")==="true"),z=ze.get(t)??0,h=ie[z]||ie[0];P.useEffect(()=>{localStorage.setItem("mastery:activeUnit",t)},[t]),P.useEffect(()=>{localStorage.setItem("mastery:code:"+t,n)},[t,n]),P.useEffect(()=>{localStorage.setItem("mastery:activeTab",o)},[o]),P.useEffect(()=>{localStorage.setItem("mastery:portalOpen",String(A))},[A]);const K=async()=>{if(h.practice.type==="css"){const{code:c}=await I(n);c&&i(c)}else if(h.practice.type==="jsx"){const{code:c}=await T(n);c&&i(c)}},V=c=>{i(c)},te=!!O[h.id],[H,ne]=P.useState(0),ae=P.useMemo(()=>Wa(h),[h]),ee=P.useMemo(()=>[...ae.guidance,...h.hints??[]],[ae,h.hints]),[u,X]=P.useState(null),[Q,re]=P.useState(!1),[de,he]=P.useState(0),[ue,ce]=P.useState(()=>Ge()),s=(c,g,S=!1)=>{const k={...ue,[c]:st(ue[c],g,S)};ot(k),ce(k)},r=c=>{e(c.id);const g=localStorage.getItem("mastery:code:"+c.id);i(g||c.practice.starterCode),d("editor"),p(null),N([]),ne(0),X(null),he(0)},v=async()=>{if(Q)return;re(!0),X(null);const c=await za(h,n,D);if(X(c),re(!1),s(h.id,c.pass),c.pass){const g={...O,[h.id]:!0};L(g),localStorage.setItem("mastery:solved",JSON.stringify(g)),Ee({particleCount:60,spread:70,origin:{y:.8}})}};P.useEffect(()=>{const c=setInterval(()=>he(g=>g+1),1e3);return()=>clearInterval(c)},[t]);const w=P.useDeferredValue(n);P.useEffect(()=>{if(h.practice.type==="jsx")D(w).then(c=>{c.code&&f(c.code)});else if(h.practice.type==="js_snippet"&&h.trackId!=="behavioural"){N([]);const c={log:(...g)=>{const S=g.map(k=>typeof k=="object"?JSON.stringify(k,null,2):String(k)).join(" ");N(k=>[...k,S])},error:(...g)=>{const S=g.map(k=>String(k)).join(" ");N(k=>[...k,`[ERROR] ${S}`])},warn:(...g)=>{const S=g.map(k=>String(k)).join(" ");N(k=>[...k,`[WARN] ${S}`])}};try{new Function("console",w)(c)}catch(g){N(S=>[...S,`Error: ${g.message}`])}}},[w,D,h.practice.type]);const R=()=>{s(h.id,!0,!0);const c={...O,[h.id]:!0};L(c),localStorage.setItem("mastery:solved",JSON.stringify(c)),Ee({particleCount:50,spread:60,origin:{y:.8}})},j=()=>{z<ie.length-1&&r(ie[z+1])},M=()=>{z>0&&r(ie[z-1])},y=Object.keys(O).reduce((c,g)=>{var S;return c+(((S=Qt.get(g))==null?void 0:S.xp)||0)},0),C=`<!doctype html><html><head><meta charset="utf-8">
