...
function parseAbbr(abbr: string): ParsedEmmet | null {
  // Support simple nested: parent>child
  if (abbr.includes('>')) {
...
export function createEmmetKeymap(): KeyBinding[] {
  const tryExpand = (view: EditorView): boolean => {
    const { state } = view;
    ...
    if (abbrMatch) {
      ...
      if (parsed) {
        ...
        // Indent all lines relative to current line position if multiline
  ...
  return [
    {
      key: 'Enter',
      run: (view: EditorView) => tryExpand(view),
    },
    {
      key: 'Tab',
      run: (view: EditorView) => tryExpand(view),
