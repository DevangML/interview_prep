...
/**
 * Tab and Enter expansion for Emmet abbreviations and tag splitting.
 */
...
/**
 * Enter:
 * 1. Checks if cursor is between matching tags <tag>|</tag> -> opens multi-line indented block.
 * 2. If preceded by an Emmet abbreviation (e.g. div.red, div.class_name, button.btn) -> expands it.
 * 3. Otherwise re-indents line being left and inserts clean indented newline.
 */
const formatOnEnter: Command = (view) => {
  const { state } = view;
  const range = state.selection.main;
  if (!range.empty || state.readOnly) return insertNewlineAndIndent(view);

  const line = state.doc.lineAt(range.head);
  const textBefore = line.text.slice(0, range.head - line.from);
  const textAfter = line.text.slice(range.head - line.from);
  ...
  if (tagMatchBefore && tagMatchAfter && tagMatchBefore[1] === tagMatchAfter[1]) {
    ...
    view.dispatch({
      ...
      selection: { anchor: range.head + 1 + nextIndent.length },
      userEvent: 'emmet',
    });
    return true;
  }

  // 2. Preceded by Emmet abbreviation like div.red or div.class_name on Enter
  const tokenMatch = /(\S+)$/.exec(textBefore);
  if (tokenMatch && /^[a-zA-Z0-9_-]+[.#][a-zA-Z0-9_#.-]+$/.test(tokenMatch[1])) {
    if (emmetExpand(view)) return true;
  }

  // 3. Normal re-indent & newline
